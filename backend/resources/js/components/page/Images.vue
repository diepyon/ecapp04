      ...
      <b-input-group class="search">
        <template #prepend>
          <b-dropdown :text="subGenreSelected.text">
            <b-dropdown-item
              @click="selectSubgenre({ value: null, text: 'すべての画像' })"
              >すべての画像</b-dropdown-item
            >
            <b-dropdown-item
  ...
  methods: {
    ...
    changePage(number) {
      ...
      if (this.subGenreSelected.value && this.keyword) {
        ...
        $url = `${window.location.origin}/image?subgenre=${this.subGenreSelected.value}&key=${this.keyword}&page=${this.current_page}`;
      } else if (this.subGenreSelected.value && !this.keyword) {
        //console.log('サブジャンルの指定はあるが、キーワードの指定はない')
        $url = `${window.location.origin}/image?subgenre=${this.subGenreSelected.value}&page=${this.current_page}`;
      } else if (!this.subGenreSelected.value && !this.keyword) {
    ...
    getSubgenre() {
      axios.get("/api/stocks/getSubgenre?genre=image").then((response) => {
