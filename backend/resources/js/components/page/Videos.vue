      ...
      <b-input-group class="search">
        <template #prepend>
          <b-dropdown :text="subGenreSelected.text">
            <b-dropdown-item
              @click="selectSubgenre({ value: null, text: 'すべての映像' })"
              >すべての映像</b-dropdown-item
            >
            <b-dropdown-item
      ...
      <div class="" v-for="stock in stocks" :key="stock.id">
        <div class="stock_thumbnail">
          <router-link :to="`stocks/` + stock.id">
            ...
            >
              <img
                @error="checkImgExist(stock.id)"
                :id="stock.id"
                v-show="previewingId != stock.id"
                class="thumbnail"
              ...
              <span v-show="previewingId == stock.id">
                <video
                  ...
                  v-show="previewingId"
                  :id="'preview' + stock.id"
                  :src="'/storage/stock_thumbnail/' + stock.filename + '.mp4'"
                  class="thumbnail"
...
export default {
  ...
  methods: {
    videoPreviewError(id, filename) {
      ...
      video.outerHTML =
        "<img class='thumbnail' src='/storage/stock_thumbnail/" +
        filename +
        ".png'" +
        ">";
    },
    ...
    changePage(number) {
      ...
      if (this.subGenreSelected.value && this.keyword) {
        ...
        $url = `${window.location.origin}/video?subgenre=${this.subGenreSelected.value}&key=${this.keyword}&page=${this.current_page}`;
      } else if (this.subGenreSelected.value && !this.keyword) {
        //console.log('サブジャンルの指定はあるが、キーワードの指定はない')
        $url = `${window.location.origin}/video?subgenre=${this.subGenreSelected.value}&page=${this.current_page}`;
      } else if (!this.subGenreSelected.value && !this.keyword) {
    ...
    getSubgenre() {
      axios.get("/api/stocks/getSubgenre?genre=video").then((response) => {
