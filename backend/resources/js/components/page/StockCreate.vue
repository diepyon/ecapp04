    ...
    <div id="form">
      ...
      <div class="form-group">
        ...
        <select v-model="feeSelected">
          <option
            v-for="feeOption in feeOptions"
            v-bind:value="feeOption.value"
            :key="feeOption.value"
          >
            {{ feeOption.text }}
          </option>
      ...
      <template v-if="genre">
        <div>
          ...
          <b-form-select
            v-model="subGenreSelected"
            :options="subGenreOption"
            class="mb-3"
            @change="checkSubGenre"
            @blur="checkSubGenre"
          >
          ...
          <div class="mt-3">
            subGenreSelected: <strong>{{ subGenreSelected }}</strong>
          </div>
        </div>
      </template>
      ...
      <div>
        ...
        <b-form-tags
          ...
          :limit-tags-text="'登録できるのは' + limit + '個までです。'"
          add-button-text="追加"
          :tag-validator="validator"
          invalid-tag-text="無効なタグ(文字数が10文字より多いなど)"
...
export default {
  ...
  methods: {
    ...
    getSubgenre() {
      axios
        .get("/api/stocks/getSubgenre?genre=" + this.genre)
        .then((response) => {
          this.subGenreOption = [];
    ...
    checkName() {
      ...
      if (n > this.maxNameLength) {
        ...
        this.errorMessage.name =
          String(this.maxNameLength) + "文字以内で入力してください。";
      } else if (n == 0) {
    ...
    checkTag() {
      ...
      if (n && n > this.maxTagLength) {
        this.errorMessage.tag =
          String(this.maxTagLength) + "文字以内で入力してください。";
    ...
    checkDetail() {
      ...
      if (n > this.maxDetailLength) {
        this.errorMessage.detail =
          String(this.maxDetailLength) + "文字以内で入力してください。";
      } else if (n == 0) {
    ...
    dropFile(event) {
      ...
      if (
        this.fileType.match("video") ||
        this.fileType.match("audio") ||
        this.fileType.match("image")
      ) {
    ...
    genreSelect() {
      ...
      if (result && this.fileInfo && this.fileInfo.type.match("image")) {
        ...
        result &&
        this.fileInfo &&
        this.fileInfo.type.match("quicktime")
      ) {
    ...
    stockCreate() {
      ...
      if (nameResult && detailResult && fileResult && subGenreReulst) {
        ...
        axios
          .post("/api/stocks/create", postData) //api.phpのルートを指定。第2引数には渡したい変数を入れる（今回は配列postData=入力された内容）
          .then((response) => {
